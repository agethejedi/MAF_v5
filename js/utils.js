...
// Addition/Subtraction bounds — scale by grade and difficulty
// By grade 4+ numbers become 2-digit multipliers, by grade 8 three-digit
const BOUNDS = {
  easy: {
    1:{min:0,  max:10},   // single digit
    2:{min:0,  max:20},   // up to 20
    3:{min:0,  max:50},   // up to 50
    4:{min:10, max:99},   // 2-digit
    5:{min:10, max:99},
    6:{min:10, max:199},
    7:{min:10, max:499},
    8:{min:10, max:999},  // 3-digit
  },
  medium: {
    1:{min:5,  max:20},
    2:{min:10, max:50},
    3:{min:10, max:100},
    4:{min:10, max:999},  // 2–3 digit
    5:{min:10, max:999},
    6:{min:10, max:9999},
    7:{min:10, max:9999},
    8:{min:100,max:99999}, // 3–5 digit
  },
  hard: {
    1:{min:10, max:20},
    2:{min:20, max:100},
    3:{min:20, max:200},
    4:{min:50, max:9999},  // 2–4 digit
    5:{min:50, max:9999},
    6:{min:50, max:99999},
    7:{min:50, max:99999},
    8:{min:100,max:999999}, // up to 6-digit
  },
};

// Multiplication tables — easy=×10, medium=×15, hard=×150
// Grade scaling: 2-digit multipliers by grade 4, 3-digit by grade 8
const MUL_BOUNDS = {
  easy: {
    1:{a:[0,5],   b:[0,5]},
    2:{a:[0,10],  b:[0,10]},
    3:{a:[0,10],  b:[0,10]},
    4:{a:[0,10],  b:[0,15]},  // 2-digit results
    5:{a:[0,10],  b:[0,15]},
    6:{a:[2,12],  b:[2,15]},
    7:{a:[2,15],  b:[2,15]},
    8:{a:[2,15],  b:[2,15]},
  ...
  medium: {
    ...
    2:{a:[0,10],  b:[0,15]},
    3:{a:[0,12],  b:[0,15]},
    4:{a:[2,15],  b:[2,15]},  // 2-digit × 2-digit
    5:{a:[2,15],  b:[2,20]},
    6:{a:[5,20],  b:[5,20]},
    7:{a:[5,25],  b:[5,25]},
    8:{a:[10,50], b:[10,50]}, // 2-digit × 2-digit larger
  },
  hard: {
    1:{a:[0,10],  b:[0,15]},
    2:{a:[0,12],  b:[0,15]},
    3:{a:[2,15],  b:[2,15]},
    4:{a:[10,50], b:[10,50]},  // 2-digit × 2-digit
    5:{a:[10,99], b:[10,50]},
    6:{a:[10,99], b:[10,99]},  // 2-digit × 2-digit
    7:{a:[10,150],b:[10,99]},
    8:{a:[10,150],b:[10,150]}, // up to 3-digit × 3-digit
  },
};
...
function mulBounds(grade, difficulty) {
  return MUL_BOUNDS[difficulty]?.[grade] || { a:[0,10], b:[0,10] };
}
...
export function makeProblem({ grade = 1, types = ['add', 'sub'], difficulty = 'easy' }) {
  ...
  if (pick === 'mul') {
    const mb = mulBounds(grade, difficulty);
    const x = randInt(mb.a[0], mb.a[1]);
    const y = randInt(mb.b[0], mb.b[1]);
    return { type: pick, question: `${x} × ${y}`, answer: x * y, mode: 'numeric' };
  }
  if (pick === 'div') {
    // Divisor scales with grade/difficulty, dividend is always a clean multiple
    const mb = mulBounds(grade, difficulty);
    const y = Math.max(1, randInt(mb.a[0] || 1, mb.a[1]));
    const quotient = randInt(1, mb.b[1]);
    const x = y * quotient;
    return { type: pick, question: `${x} ÷ ${y}`, answer: quotient, mode: 'numeric' };
  }

  // ── Fractions (Grades 4+) ─────────────────────────────────
  if (pick === 'fraction') {
    const maxDen = difficulty === 'easy' ? 6 : difficulty === 'medium' ? 10 : 12;
    ...
    if (type === 0) {
      ...
      if (sum < den) {
        return { type: pick,
          ...
          answer: `${sum}/${den}`,
          mode: 'fraction',
          hint: `Add the numerators and keep the denominator: ${num1} + ${num2} = ${sum}`
        };
      }
      // Improper — simplify
      const [sn, sd] = simplify(sum, den);
      return { type: pick,
        question: `What is ${num1}/${den} + ${num2}/${den}? Simplify your answer.`,
        answer: sd === 1 ? `${sn}` : `${sn}/${sd}`,
        mode: 'fraction',
        hint: `Add numerators: ${num1}+${num2}=${sum}, then simplify ${sum}/${den}`
      };
    } else if (type === 1) {
      // Same denominator subtraction (ensure positive result)
      const big = Math.max(num1, num2), small = Math.min(num1, num2);
      if (big === small) {
        return { type: pick,
          question: `What is ${big}/${den} - ${small}/${den}?`,
          answer: `0`,
          mode: 'fraction',
          hint: `When numerators are equal the answer is 0`
        };
      }
      const diff = big - small;
      const [sn, sd] = simplify(diff, den);
      return { type: pick,
        question: `What is ${big}/${den} - ${small}/${den}?`,
        answer: sd === 1 ? `${sn}` : `${sn}/${sd}`,
        mode: 'fraction',
        hint: `Subtract the numerators: ${big} - ${small} = ${diff}`
      };
    } else {
      // Compare fractions
      const den2 = randInt(2, maxDen);
      const num3 = randInt(1, den2 - 1);
      const val1 = num1 / den, val2 = num3 / den2;
      const symbol = val1 > val2 ? '>' : val1 < val2 ? '<' : '=';
      return { type: pick,
        question: `Compare: ${num1}/${den} ___ ${num3}/${den2}. Which symbol goes in the blank?`,
        answer: symbol,
        choices: ['>', '<', '='],
        mode: 'choice',
        hint: `Convert to decimals: ${num1}/${den} = ${(val1).toFixed(2)}, ${num3}/${den2} = ${(val2).toFixed(2)}`
      };
    }
  }
  ...
  if (pick === 'rounding') {
    const places = grade <= 2
      ...
      : grade <= 5 ? [10, 100, 1000]
      : [100, 1000, 10000];
    // Hard difficulty uses larger numbers
    const place = places[randInt(0, places.length - 1)];
    const multiplier = difficulty === 'easy' ? 5 : difficulty === 'medium' ? 10 : 20;
    const n = randInt(place * 2, place * multiplier);
  ...
  if (pick === 'compare') {
    const n = grade <= 2
      ? randInt(b.min || 1, b.max)
      : grade <= 3 ? randInt(100, difficulty === 'hard' ? 9999 : 999)
      : grade <= 5 ? randInt(1000, difficulty === 'hard' ? 99999 : 9999)
      : randInt(10000, difficulty === 'hard' ? 999999 : 99999);
    const diff = randInt(1, Math.max(1, Math.floor(n * 0.1)));
